...
const flightForm = ({ options, optionLabel, type }) => {
  ...
  const handleSubmit = (event) => {
    if (departureInputRef.current.value && returnInputRef?.current?.value) {
      let result = dateCompare(
        departureInputRef.current.value,
        returnInputRef?.current?.value
      );
      if (!result) {
        alert(
          "please fill the correct dates in departure and return input fields"
        );
        return;
      }
    }
    if (fromInputRef.current.value === toInputRef.current.value) {
      alert(
        "please fill the correct location name in 'From' and 'To' input fields"
      );
      return;
    }
